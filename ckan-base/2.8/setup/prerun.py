...
def add_views():
    views = os.environ.get('CKAN__VIEWS')
    
    if views:
        cmd = ['paster', '--plugin=ckan', 'config-tool', ckan_ini, 'ckan.views.default_views = image_view text_view recline_view {}'.format(views)]

        subprocess.call(cmd)
        print '[prerun] Default views: {} have been added to the ini file.'.format(views)
